...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    UIColor *mainColor;
    if (![[NSUserDefaults standardUserDefaults] valueForKey:@"mainColor"]) {
        mainColor = [UIColor colorWithHue:0.0f / 360.0f
                               saturation:0.79f
                               brightness:1.0f
                                    alpha:1.0f];
        [[ZTZhyk sharedInstance] saveMainColor:mainColor];
    } else {
        mainColor = [[ZTZhyk sharedInstance] loadMainColor];
    }
